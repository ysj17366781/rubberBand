...
{
    ui->setupUi(this);
    m_plotter = new Plotter(ui->centralWidget);
}
...
Plotter::Plotter(QWidget *parent):QWidget(parent)
{
    ...
    for (int var = 0; var < total; ++var) {
        QPointF pf(var*2*pi/100*settings.spanX(), 5+std::cos(var*2*pi/100*settings.spanX()));
        vec.append(pf);
    }
    setCurveData(0, vec);
    vec.clear();
    for (int var = 0; var < total; ++var) {
        QPointF pf(var*2*pi/100*settings.spanX(), 5+std::sin(var*2*pi/100*settings.spanX()));
        vec.append(pf);
    }
    setCurveData(1, vec);
}
